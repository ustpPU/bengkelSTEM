...
type View = "dashboard" | "register" | "report" | "materials";
...
type DashboardData = { registered:number; reports:number; scienceRegistered:number; mathRegistered:number; uniqueSchools:number };
...
function Dashboard({ go }: { go: (view: View) => void }) {
  const [data, setData] = useState<DashboardData>({registered:0,reports:0,scienceRegistered:0,mathRegistered:0,uniqueSchools:0});
  ...
  const stats = [
    ...
    { label: "Pendaftaran / Kehadiran", value: String(data.registered), note: "Direkod semasa pendaftaran", tone: "blue" },
    { label: "Sekolah terlibat", value: String(data.uniqueSchools), note: "Berdasarkan peserta berdaftar", tone: "green" },
    ...
    <section className="hero">
      <div className="hero-copy">
        ...
        <div className="hero-actions">
          ...
          <button className="secondary" onClick={() => go("materials")}><Icon name="book"/> Semak bahan bengkel</button>
        </div>
      </div>
    ...
    <section className="content">
      ...
      <div className="dashboard-grid">
        <article className="panel progress-panel">
          ...
          <div className="day-progress"><div className="day-label"><div><i className="science"/><b>Ketua Panitia Sains</b></div><strong>{data.scienceRegistered} <span>/ 70</span></strong></div><div className="bar"><i className="science-bar" style={{width:`${Math.min(100,data.scienceRegistered/70*100)}%`}}/></div><small>12 Ogos 2026 · pendaftaran dikira sebagai kehadiran</small></div>
          <div className="day-progress"><div className="day-label"><div><i className="math"/><b>Ketua Panitia Matematik</b></div><strong>{data.mathRegistered} <span>/ 70</span></strong></div><div className="bar"><i className="math-bar" style={{width:`${Math.min(100,data.mathRegistered/70*100)}%`}}/></div><small>13 Ogos 2026 · pendaftaran dikira sebagai kehadiran</small></div>
        </article>
...
function FormShell({ type, go }: { type: "register" | "report"; go: (view: View) => void }) {
  ...
  const title = type === "register" ? "Pendaftaran peserta" : "Laporan peserta";
  const subtitle = type === "register" ? "Lengkapkan maklumat seperti dalam rekod rasmi sekolah. Pendaftaran ini turut dikira sebagai rekod kehadiran bengkel." : "Pilih nama yang telah didaftarkan dan lengkapkan laporan pelaksanaan.";
  ...
  return <section className="form-page">
    ...
    <div className="form-wrap"><div className="form-intro"><span className="eyebrow">Bengkel STEM AI · PPDPU</span><h1>{title}</h1><p>{subtitle}</p><div className="mini-event"><b>12 & 13 Ogos 2026</b><span>Dewan Sekolah Rendah Sri KDU</span></div></div>
      <form onSubmit={submit}>
        ...
        <button className="primary submit" type="submit" disabled={loading}>{loading ? "Menghantar..." : type === "register" ? "Hantar pendaftaran" : "Hantar laporan"} <span>→</span></button>
...
export default function Home() {
  ...
  const [view, setView] = useState<View>(queryView === "materials" ? "materials" : queryView === "report" ? "report" : queryView === "register" ? "register" : "dashboard");
  const go = (next: View) => { setView(next); window.scrollTo({top:0, behavior:"smooth"}); };
  return <main>
    <header className="site-header"><button className="brand" onClick={() => go("dashboard")}><span className="brand-mark">AI</span><span><b>STEM AI</b><small>PETALING UTAMA</small></span></button><nav><button className={view === "dashboard" ? "active" : ""} onClick={() => go("dashboard")}>Dashboard</button><button className={view === "register" ? "active" : ""} onClick={() => go("register")}>Pendaftaran</button><button className={view === "materials" ? "active" : ""} onClick={() => go("materials")}>Bahan</button><button className={view === "report" ? "active" : ""} onClick={() => go("report")}>Laporan</button></nav><span className="agency">PPD<br/><b>PETALING UTAMA</b></span></header>
